...
本仓库的插件清单 `index/plugins.json` 由社区通过 PR 维护。作者给**自己的插件**提 PR 申请**收录**；收录之后，CI 会自动生成对应的「整合包必备」徽章。
...
| `badges` | 否 | 徽章数组，当前仅 `"essential"`；CI 据此自动生成徽章 |

## 徽章「整合包必备」（essential）

插件**被收录后**，CI 会自动生成徽章。徽章贴在插件 README 上，读作「**这个插件是整合包必备插件**」。它代表「**强烈推荐**」：打造 DSH 整合包时，这个插件是我们十分推荐放进包里的——不是「离了它就不能用」的硬性必需，而是推荐度很高的意思。
